...
main() {
    ...
    if [ -f "$MODULE_PROP" ]; then
        ...
        ui_print "主模块：$MODULE_NAME $MODULE_VERSION"
    fi
...
if [ ! -f "$SYSTEM_PROP_FILE" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "警告: 无法创建 $SYSTEM_PROP_FILE，跳过哈希写入，不终止安装"
    fi
fi

# 容错：HASH_VALUE为空时不报错退出，仅提示
if [ -z "$HASH_VALUE" ]; then
    echo "警告：当前环境无法读取ro.boot.vbmeta.digest哈希，仅写入verity配置"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "警告: 无法写入哈希，继续写入verity配置，不终止安装"
    fi
...
if [ -x "$AVBCTL" ]; then
    "$AVBCTL" disable-verity --force >/dev/null 2>&1 || true
    "$AVBCTL" disable-verification --force >/dev/null 2>&1 || true
else
    echo "警告: 未找到 avbctl，跳过 disable-verity"
fi
